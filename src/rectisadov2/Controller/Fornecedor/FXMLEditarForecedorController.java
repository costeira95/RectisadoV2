 ...
 * @author Costeira
 */
public class FXMLEditarForecedorController extends Stage {
    ...
    public FXMLEditarForecedorController(AppStart appStart, TableView<Cliente> tblView, Stage stage) {
    ...
    void handleEditar(MouseEvent event) {
        ...
        Gestor.getInstanceFornecedores().getDAO().updateUtilizador(cliente);
        List<Cliente> clientes = Gestor.getInstanceFornecedores().getElements();
        clientes.sort(Cliente.comparator);
        tblView.setItems(FXCollections.observableArrayList(clientes));
        new FXMLDialogSucessoController(appStart, this, "Fornecedor editado com sucesso!");
    }
